...
public class Service implements Initializable{
	...
	public void creerSalarier() {
		try{  			
			...
			System.out.println("entrer dateEmbauche:");  
			double dateE=Double.parseDouble(br.readLine());
			System.out.println("entrer valeur supplémentaire:");  
			...
			if(categorie=="employe") {
				...
				System.out.println(i+" vendeur ajouté dans entreprise");  
			}
	...
	public void initialize(URL arg0, ResourceBundle arg1) {
		...
		colCategorie.setCellValueFactory(new PropertyValueFactory<>("catego"));
	...
	public void refresh() {
		...
		try {
			...
			while(rs.next()) {
				...
				v.setSalaireF(rs.getDouble(4));
				listSal.add(v);
				listCat.add( new Cat( rs.getString(8)));
			}
		...
		ObservableList<Cat> obC = FXCollections.observableArrayList(listCat);
		tableau.setItems(ob);
		tableauCat.setItems(obC);
	}
	
	@FXML
	TableView<Cat> tableauCat;
	...
	@FXML
	TableColumn<Cat, String> colCategorie;
	public void ajout() {
		try{
			PreparedStatement ps=connexion.getCon().prepareStatement("insert into entreprise values(?,?,?,?,?,?,?,?)");  			
	...
	public void filtrer() {
		if(minimum.getText().length()==0 && maximum.getText().length()==0) {
			...
			a.setTitle("Rod belk");
			a.setContentText("Merci pour insérer deux valeur à comparer");
			a.show();
		}else {
	...
	public void ListerparSalaire(){
		listSal = new ArrayList<vendeur>();
		listCat = new ArrayList<Cat>();
		String req = "select * from entreprise order by salaire desc";
		try {
			Statement stmt = connexion.getCon().createStatement();
			ResultSet rs = stmt.executeQuery(req);
			while(rs.next()) {
